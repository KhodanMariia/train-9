...
function findIndexByProperty(arr, name) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const isName = obj => obj.name === name;
...
function removeAndDouble(arr, startIdx) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const newArray = arr.splice(startIdx);

    // За допомогою методу map() подвоюємо кожен елемент масиву.
    const newArray1 = newArray.map(x => x * 2);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const newArr1 = arr.slice(n);

    // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
    const newArr2 = newArr1.slice(-n);

    const newArr3 = newArr2.reduceRight(
      (accumulator, currentValue) => accumulator + currentValue
    );
...
function wordLengths(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const newArr1 = arr.map(element => element.length);
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) && n !== "number") {
    ...
    //Створюю новий масив куди будуть добавлятися елементи з циклу повторень
    const newArr1 = [];

    arr.forEach(number => {
      // Додаємо кожен елемент до нового масиву n разів
      for (let i = 0; i < n; i++) {
        newArr1.push(number);
      }
    });
...
function findLongestWord(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    //метод reduce() виводить кінцевий результат, тобто останій елемент
    return arr.reduce((longestWord, currentWord) => {
      ...
      return currentWord.length > longestWord.length
        ? currentWord // Інакше повертаємо попереднє найдовше слово без змін
        : longestWord; // Повертаємо найдовше слово
    });
  }
}
...
function findDuplicateElements(arr) {
  ...
  return arr.filter((element, index) => {
    ...
    const duplicateElements = arr.indexOf(element) !== index;
...
function findLastNegativeNumberIndex(arr) {
  ...
  const isLargeIndex = index => index <= arr.length;

  return arr.findLastIndex(isLargeIndex);
